...
jest.mock("../../src/models/shop.model.js", () => ({
  ...
  ShopifyShop: {
    // Additional mock setup for ShopifyShop if required
  },
}));
jest.mock("bcryptjs", () => ({
  genSalt: jest.fn(),
  hash: jest.fn(), // Mock other methods you might use
}));
const bcrypt = require("bcryptjs");
const { Shop, ShopifyShop } = require("../../src/models/shop.model.js");
const { authenticateShop } = require("../../src/services/channel/shopify.service");
...
describe("Shopify Service", () => {
  describe("authenticateShop", () => {
    beforeEach(() => {
      ...
      process.env.SERVER_BASE_URL = "http://localhost:80";
      process.env.SHOPIFY_CLIENT_ID = "test_client_id";
      Shop.findOne.mockReset();
      Shop.findOne.mockResolvedValue({ name: "testShop", organizationId: "org1" });
    });

    it("should return organizations URL if shop exists and has organizationId", async () => {
      Shop.findOne.mockResolvedValue({ name: "testShop", organizationId: "org1" });

      const result = await authenticateShop("testShop");
      console.log("result====".green.bold.underline);
      console.log(result);

      expect(result).toEqual({ url: `${process.env.CLIENT_BASE_URL}/organizations` });
    });

    it("should return channel management URL if shop exists and does not have organizationId", async () => {
      Shop.findOne.mockResolvedValue({ name: "testShop" });

      const result = await authenticateShop("testShop");

      expect(result).toEqual({
        url: `${process.env.CLIENT_BASE_URL}/channel-management?shop=testShop&install=true&channel=shopify`,
      });
    });

    it("should return install URL if shop does not exist", async () => {
    ...
    it("should throw error if salt generation fails", async () => {
      ...
      await expect(authenticateShop("testShop")).rejects.toThrow("Salt generation error");
    });
  });
});
